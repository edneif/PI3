...
extern uint16_t media_adc[4];
extern char times[30];
extern char dates[30];

extern void processa_mensagem_recebida(void);
				...
				if (accept_err == ERR_OK)
				{
					...
					while (netconn_recv(newconn, &buf) == ERR_OK)
					{
						...
						do
						{
							...
							processa_mensagem_recebida();


							// Or modify the message received, so that we can send it back to the client

							//int len = sprintf (smsg, "\"%s\" was sent by the Server\n", msg);
							//int len = sprintf (smsg, "%i  %i  %i  %i %s %s was sent by the Client\n", media_adc[0],
							//							media_adc[1], media_adc[2] ,media_adc[3],dates, times);

							int len = sprintf (smsg, "%i ; %s ; %s", media_adc[0],dates,times);
